...
void tb3::transform_callback() {
  if (!m_go_to_goal && !m_go_to_angle) return;

  geometry_msgs::msg::TransformStamped t;

  try {
    t = m_tf_buffer->lookupTransform(m_parent_frame, m_child_frame,
                                     tf2::TimePointZero);
...
double tb3::compute_distance(const std::pair<double, double>& a,
...
void tb3::go_to_goal_callback() {
  ...
  if (m_go_to_goal == false && m_go_to_angle == true) {
    ...
    double w =
        angle_to_goal;  // m_goal_orientation - compute_yaw_from_quaternion();
...
int main(int argc, char* argv[]) {
  ...
  // node
  auto node = std::make_shared<tb3>();
  std::vector<std::vector<float>> goals = {
      {-4.0, 0.0, 3.14}, {-4.0, 0.0, 4.9}, {0.0, 0.0, 3.14}, {0.0, 0.0, 4.9}};
  node->set_goal(goals);
  rclcpp::spin(node);
  // shutdown
  rclcpp::shutdown();
