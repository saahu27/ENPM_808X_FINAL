...
void MinimalSubscriber::topic_callback(
    const tutorial_interfaces::msg::Num::SharedPtr msg)  // CHANGE
{
  if (abs((float)ang) <= 0.0001 && abs((float)lin_x) <= 0.0001 &&
      abs((float)lin_y) <= 0.0001) {
...
void MinimalSubscriber::robot_pose_callback(
    const nav_msgs::msg::Odometry::SharedPtr pose_msg)  // CHANGE
{
