...
class TaskController : public testing::Test {
 public:
...
void TaskController::callback() {
  auto message = ODOM();
  test_pub->publish(message);
}

TEST_F(TaskController, test_search_bins) {
  node_ = rclcpp::Node::make_shared("test_controller");

  auto test_pub = node_->create_publisher<ODOM>
                    ("odom", 10.0);
  auto ypos = 3.0;
  TIMER timer = node_->create_wall_timer(100ms,
      std::bind(&TaskController::callback, this));
...
TEST_F(TaskController, test_resume_search) {
  node_ = rclcpp::Node::make_shared("test_controller");
...
TEST_F(TaskController, test_move_to_disposal) {
  node_ = rclcpp::Node::make_shared("test_controller");
